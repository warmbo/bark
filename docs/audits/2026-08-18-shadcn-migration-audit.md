...
- **CSS scale:** 4,180 source lines (legacy 3,481 + theme 84 + v3 615). `legacy.css` had **852 distinct class selectors**, 15 `!important`, and ~40 hardcoded hex/rgba values. `v3.css` (615) is the new shadcn-utility layer. `main.css` = merged output.
  - **Update (css-migration branch):** `legacy.css` is **retired**. Its `:root` token block was hoisted to `frontend/src/tokens.css` (single source of truth) and its component rules were tokenized (all hardcoded hex/rgba → semantic `--c-*` tokens with verbatim values) and moved to `frontend/src/components.css`. Cruft `!important` was removed; a11y/reduced-motion `!important` retained. Build = `tailwind(theme) + tokens + components + v3`.
