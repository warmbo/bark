...
# ── bot_appearance wallpaper invert (owner-only + persistence) ────────────


@pytest.mark.asyncio
async def test_bot_appearance_wallpaper_rejects_non_owner(app, monkeypatch):
    """The wallpaper invert treatment is INSTANCE-GLOBAL — only owners may set it."""
    from dashboard.routes.api.bot_appearance import update_wallpaper

    request = MagicMock()
    request.session = {"user": {"id": "43"}, "role": "admin"}  # not the owner
    resp = await update_wallpaper(request, "1")
    assert resp.status_code == 403


@pytest.mark.asyncio
async def test_bot_appearance_wallpaper_sets_and_persists(app, monkeypatch, tmp_path):
    ...
    request = MagicMock()
    request.session = {"user": {"id": "42"}, "role": "admin"}  # owner
    request.state.bot = MagicMock()

    request.json = AsyncMock(return_value={"invert": True})
    resp = await update_wallpaper(request, "1")
    assert resp.status_code == 200
    assert is_wallpaper_inverted(tmp_path) is True

    request.json = AsyncMock(return_value={"invert": False})
    resp = await update_wallpaper(request, "1")
    assert resp.status_code == 200
    assert is_wallpaper_inverted(tmp_path) is False


@pytest.mark.asyncio
async def test_bot_appearance_returns_wallpaper_invert(app, monkeypatch, tmp_path):
    from config import config
    from dashboard.routes.api.bot_appearance import get_bot_appearance
    ...
    save_wallpaper(tmp_path, True)
    resp = await get_bot_appearance(request, "1")
    assert resp.status_code == 200
    data = json.loads(bytes(resp.body).decode("utf-8"))["data"]
    assert data["wallpaper_invert"] is True
