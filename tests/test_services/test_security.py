...
def test_module_action_path_only_matches_runtime_actions():
    assert _module_action_from_path(
        "/api/v1/guilds/123/modules/logging/test"
    ) == (123, "logging", "test")
    assert _module_action_from_path(
        "/api/v1/guilds/123/modules/logging/toggle"
    ) is None
    assert _module_action_from_path(
        "/api/v1/guilds/123/modules/logging"
    ) is None
...
async def test_cross_origin_api_write_is_rejected():
    ...
    async with AsyncClient(
        ...
        response = await client.post(
            "/api/v1/save", headers={"Origin": "https://evil.example"}
        )
...
@pytest.mark.asyncio
async def test_public_https_configuration_emits_hsts():
    app = FastAPI()
    app.add_middleware(SecurityMiddleware)
...
def test_mutation_capabilities_cover_every_route_family_and_gets_remain_readable():
    ...
    assert mutation_capability("POST", "/api/v1/guilds/1/modules/roles/create_role_menu") == "roles.manage"
