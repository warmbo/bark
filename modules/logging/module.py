...
class LoggingModule(BarkModule):
    ...
    def get_about(self) -> list[dict]:
        return [
            ...
            {
                "title": "Configuration",
                "description": f"Use /{self.command_group_name()} logsetup or the dashboard Configuration section to set per-event-type logging channels. Each event type (message_edit, message_delete, file_upload, member_join, member_leave, voice_state) can have its own channel.",
            },
        ]
