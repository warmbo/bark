...
class SlashDispatcher:
    ...
    def build_module_commands(self, module_names: list[str] | None = None) -> list[Any]:
        """Construct one native top-level command PER MODULE (the "other bots"
        UX): users get ``/birthday set <day> <month>`` with real typed fields
        instead of the flat ``/bark`` string command/args options.

        Pass ``module_names`` to build only a subset (used by the module
        manager as modules load incrementally); omit for the full surface.

        Mapping:
          - Single-command modules register their leaf directly at the top
            level (``/announce``, ``/roles``, ``/speak``, ``/welcome``).
          - Group-structured modules (e.g. birthdays → ``birthday set``)
            register a top-level Group named after the leading path token
            (``/birthday set|remove|list|channel``).
          - Flat multi-command modules register a Group named after the module
            (``/moderation warn``, ``/reputation leaderboard``).

        Every leaf carries the module enablement gate (see ``_native_leaf``), so
        a disabled module's commands politely reject instead of erroring. Each
        top-level command stays well under Discord's 8000-byte payload and 25-
        child caps because the surface is spread across ~12 commands instead of
        one monolithic ``/bark`` group (2026-09-01 tree-sync incident).

        Alias groups are intentionally NOT mirrored natively — the flat
        ``/bark`` fallback still resolves ``/bark rep lb`` style aliases, and
        native UX favors one canonical surface per command.
        """
        commands: list[Any] = []

        def _module_namespace(module_name: str, leaves: list[Leaf]) -> str:
            """Pick the natural top-level command name for a module."""
            firsts = [p.split()[0] for p in (leaf.path for leaf in leaves)]
            if firsts and all(f == firsts[0] for f in firsts):
                return firsts[0]  # group-structured: 'birthday set' -> 'birthday'
            return module_name

        names = module_names if module_names is not None else sorted(self._module_paths)
        for module_name in sorted(names):
            leaves = [self._registry[p] for p in self._module_paths.get(module_name, [])]
            if not leaves:
                continue
            if len(leaves) == 1:
                # Single-command module -> top-level leaf command.
                leaf = leaves[0]
                try:
                    commands.append(self._native_leaf(leaf))
                except Exception:
                    logger.exception("Failed to register %s natively", leaf.path)
                continue
            name = _module_namespace(module_name, leaves)
            if name == "help":
                # Help's leaves (help/info/stats) are distinct top-level
                # commands, not a "help" group.
                for leaf in leaves:
                    try:
                        commands.append(self._native_leaf(leaf))
                    except Exception:
            ...
            for leaf in leaves:
                ...
                try:
                    ...
                    logger.exception("Failed to add %s to native group", leaf.path)
            try:
                commands.append(sub)
            except Exception:
                logger.exception("Failed to register native group %s", name)
        return commands
