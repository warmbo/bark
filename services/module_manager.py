...
class ModuleManager:
    ...
    def discover(self) -> None:
        """Scan the modules package for BarkModule subclasses (delegated)."""
        self._discovery.discover()

    def discover_plugins(self) -> None:
        """Load single-file plugins from the plugins directory (delegated)."""
        self._discovery.discover_plugins()

    def _reload_module_package(self, package_name: str) -> BarkModule | None:
        """Reload one package's Python modules (delegated to ModuleDiscovery)."""
        return self._discovery.reload_package(package_name)
